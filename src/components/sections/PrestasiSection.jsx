...
const prestasi = [
  ...
  { title: "Pengawasan Kearsipan A", agency: "ANRI", year: "2025", icon: "bi-award", color: "bg-accent-500" },
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
          {prestasi.map((p, i) => (
            <div key={i} className="bg-white p-8 rounded-[2.5rem] border border-accent-100 shadow-sm hover:-translate-y-2 transition-all flex flex-col items-center group">
              ...
              <p className="text-[10px] font-bold text-accent-500 uppercase mb-4 tracking-tighter">{p.agency}</p>
              <span className="bg-accent-50 text-primary text-[10px] font-black px-4 py-1 rounded-full border border-accent-100">{p.year}</span>
            </div>
          ))}
        </div>
      </div>
